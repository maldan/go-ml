...
class GoRenderWasm {
  ...
  static async init(wasmData: ArrayBuffer) {
    ...
    const step = (timestamp: number) => {
      ...
      // Check sound
      if ((window as any).MegaAudio._player) {
        for (let i = 0; i < 4; i++) {
          let goshan = (window as any).go;
          let audioCommandPtr =
            goshan.pointer.audioCommandList + i * (32 + 8 + 4 + 4);
          ...
          if (isSet) {
            ...
            let volume = goshan.memory.readF32(
              audioCommandPtr + 8 + 8 + 8 + 8 + 8
            );
            let pitch = goshan.memory.readF32(
              audioCommandPtr + 16 + 16 + 8 + 4
            );
